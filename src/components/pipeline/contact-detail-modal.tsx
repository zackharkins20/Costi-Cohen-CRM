        ...
        <SheetContent className="bg-[#0a0a0a] border-[#222222] overflow-y-auto w-full sm:max-w-xl">
          <SheetHeader>
            <SheetTitle className="text-white flex items-center gap-3">
              ...
              <div>
                ...
                {contact.company && (
                  <span className="block text-xs font-normal text-[#A0A7AB]">{contact.company}</span>
                )}
              </div>
            </SheetTitle>
          </SheetHeader>
          ...
          <div className="mt-5 space-y-5 px-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label className="text-[#555555] text-[10px] uppercase tracking-wider">Email</Label>
                {editing ? (
                  <Input value={form.email || ''} onChange={e => setForm({ ...form, email: e.target.value })} className="mt-1" />
                ) : (
                  <p className="text-sm text-[#A0A7AB]">{contact.email || '—'}</p>
                )}
              </div>
              <div>
                <Label className="text-[#555555] text-[10px] uppercase tracking-wider">Phone</Label>
                {editing ? (
                  <Input value={form.phone || ''} onChange={e => setForm({ ...form, phone: e.target.value })} className="mt-1" />
                ) : (
                  <p className="text-sm text-[#A0A7AB]">{contact.phone || '—'}</p>
                )}
              </div>
            </div>
            ...
            <div className="p-4 bg-[#111111] border border-[#222222]">
              <h4 className="text-xs font-medium text-white mb-3">Property Brief</h4>
              <div className="grid grid-cols-3 gap-3 text-xs">
                <div>
                  <span className="text-[#555555]">Asset Type</span>
                  {editing ? (
                    <Input value={form.asset_type || ''} onChange={e => setForm({ ...form, asset_type: e.target.value })} className="mt-0.5 h-7 text-xs" />
                  ) : (
                    <p className="text-[#A0A7AB]">{contact.asset_type || '—'}</p>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  {editing ? (
                    ...
                    <p className="text-[#A0A7AB]">{contact.fee_percentage ? `${contact.fee_percentage}%` : '—'}</p>
                  )}
                </div>
              </div>
              {editing ? (
                ...
                <p className="text-xs text-[#A0A7AB] mt-3">{contact.brief_notes}</p>
              ) : null}
            </div>

            {/* Notes */}
            <div>
              <Label className="text-[#555555] text-[10px] uppercase tracking-wider">Notes</Label>
              {editing ? (
                <Textarea value={form.notes || ''} onChange={e => setForm({ ...form, notes: e.target.value })} className="mt-1 min-h-[60px]" />
              ) : (
                <p className="text-sm text-[#A0A7AB] mt-1">{contact.notes || '—'}</p>
              )}
            </div>
            ...
            <div>
              ...
              {docLinks.map(dl => (
                <div key={dl.id} className="flex items-center gap-2 py-1.5">
                  ...
                  <button onClick={() => { deleteDocumentLink(dl.id); getDocumentLinks('contact', contact.id).then(setDocLinks) }} className="text-[#555555] hover:text-white ml-auto">
